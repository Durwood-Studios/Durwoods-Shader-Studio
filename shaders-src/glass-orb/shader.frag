...
// Refracted colour for a single channel
float refractChannel(vec2 uv, vec2 n, float ior, float spread) {
  vec2 r = refract(normalize(vec3(0.0, 0.0, -1.0)), normalize(vec3(n, sqrt(1.0 - dot(n,n)))), 1.0 / (ior + spread)).xy;
  vec2 bgUV = uv + r * 0.45;
  // sample background hue at refracted position
  return dot(background(bgUV), vec3(0.333));
}
...
void main() {
  ...
  vec2 orbCenter = mouse * 0.08; // subtle parallax
  ...
  if (dist < uRadius) {
    ...
    // Refraction — chromatic dispersion across R/G/B channels
    float r = refractChannel(uv, p / uRadius, uIOR, -uDispersion);
    float g = refractChannel(uv, p / uRadius, uIOR, 0.0);
    float b = refractChannel(uv, p / uRadius, uIOR,  uDispersion);

    // Map sampled luminance back through a tinted hue per channel
    vec3 refracted = vec3(
    ...
    // Specular highlight — single light from upper-left
